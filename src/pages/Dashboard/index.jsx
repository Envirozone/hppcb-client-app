...
const Dashboard = () => {
  ...
  const fetchAndSetData = async () => {
    try {
      let url = `${window.apiURL}/industries_status?`;

      if ("Himachal Pradesh" && filterSelectedCategory) {
        url = `${url}state=${"Himachal Pradesh"}&industry_category=${filterSelectedCategory}`;
      } else if ("Himachal Pradesh") {
        url = `${url}state=${"Himachal Pradesh"}`;
    ...
    (async () => {
      try {
        const result = await axios(
          `${window.apiURL}/industries?state=${"Himachal Pradesh"}`
        );
        let states = [];
        let categories = [];
        let industriesName = [];
  ...
  return (
    <div className="container-fluid p-3">
      <div className="row g-3">
        ...
        <div className="col-12">
          <div className="row g-3">
            <InfoContainer allIndustriesinfo={allIndustriesinfo} />
          </div>
        </div>
        <div className="col-12">
